...
export async function GET() {
  try {
    const res = await fetch(SOMNIA_GRAPHQL, {
      ...
      body: JSON.stringify({ query: QUERY, operationName: "LatestPerpPool", variables: {} }),
      cache: "no-store"
    });
    ...
    if (json?.errors?.length) {
      return new Response(`upstream_graphql_error: ${JSON.stringify(json.errors).slice(0, 300)}`, {
        status: 502,
        headers: { "content-type": "text/plain; charset=utf-8", "access-control-allow-origin": "*" }
      });
    }

    const node = json?.data?.perpPools?.[0];
    const value = node?.totalTrade;

    if (!value) {
      return new Response(`upstream_missing_value: ${JSON.stringify(json).slice(0, 300)}`, {
        status: 502,
        headers: { "content-type": "text/plain; charset=utf-8", "access-control-allow-origin": "*" }
      });
    }
